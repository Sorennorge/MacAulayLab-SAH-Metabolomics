# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 14:44:03 2023
...
df_overview = df_overview[['Group C Mean','Group C SD','Group D Mean','Group D SD']]
df_overview = df_overview.rename(columns=({'Group C Mean':'Control Mean',
                                           'Group C SD':'Control SD',
                                           'Group D Mean':'SAH Mean',
                                           'Group D SD':'SAH SD'}))

Mean_SD_list = ['Control Mean','Control SD','SAH Mean','SAH SD']
...
df_merged['SAH Mean'] = df_merged['SAH Mean'].round(mean_decimals).apply('{0:.2f}'.format)

# Round SD values #
df_merged['Control SD'] = df_merged['Control SD'].round(sem_decimals).apply('{0:.2f}'.format)
df_merged['SAH SD'] = df_merged['SAH SD'].round(sem_decimals).apply('{0:.2f}'.format)
...
#df_meta.to_excel(os.path.join(Folder4,File4),sheet_name='Group overview',index=False)
#df_merged.to_excel(os.path.join(Folder4,File4),sheet_name='Significant lipids')
