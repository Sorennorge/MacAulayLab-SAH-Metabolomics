...
Folder1 <- "Data/Plot data/Volcano"
Folder2 <- "Results/Volcano/Images"
Folder3 <- "Results/Volcano/Data"
...
File2 = "Volcano_plot_Control_to_SAH_Grouped.png"
File3 = "Volcano_plot_Control_to_SAH_Grouped_labeled.png"
...
data <- data %>% 
  mutate(
    Expression = case_when(Log2FC >= 0 & Padj <= 0.1 ~ "Up-regulated",
                           Log2FC < 0 & Padj <= 0.1 ~ "Down-regulated",
                           TRUE ~ "Unchanged")
  )
...
p1 <- ggplot(data %>%
  ...
  geom_hline(yintercept=-log10(0.05),linetype = "dashed", col="grey50")+
  geom_hline(yintercept=-log10(0.0035),linetype = "dashed", col="grey50")+
...
write.csv2(top_genes,file.path(Folder3, File4), row.names = FALSE)

data <- data %>% 
  mutate(
    Interest_group = case_when(Log2FC > 2.5 & Pvalue < 0.05 ~ "Interesting",
                               Log2FC <= 2.5 & Pvalue < 0.05 ~ "not_Interesting",
                               TRUE ~ "Unchanged")
  )


Interested_label = subset(data,Interest_group=="Interesting")
write.csv2(Interested_label,file.path(Folder3, File5), row.names = FALSE)

p3 <- ggplot(Interested_label %>%
               arrange(match(Groups, rev(Order_list))),
             aes(Log2FC , -log(Pvalue,10))) +
  geom_vline(xintercept=c(2.5),linetype = "dashed" ,col="grey50") +
  geom_hline(yintercept=-log10(0.05),linetype = "dashed", col="grey50")+
  geom_hline(yintercept=-log10(0.0075),linetype = "dashed", col="grey50")+
  geom_point(aes(fill = Groups), size = 4,shape=21)+
  scale_x_continuous(name=expression("log"[2]*"FC"))+
  ylab(expression("-log"[10]*"pvalue")) +
  #scale_fill_manual(values = c("dodgerblue3", "gray80", "firebrick3")) +
...
p4 <-  p3 +
  geom_label_repel(data = Interested_label,
                   mapping = aes(Log2FC, -log(Pvalue,10), label = Compounds),
                   size = 2)+theme(legend.position="none")
p4

ggsave(file=file.path(Folder2, File6),plot=p3,width = 8, height = 4,units = "in", bg = "transparent",dpi=1200)
ggsave(file=file.path(Folder2, File7),plot=p4,width = 8, height = 4,units = "in", bg = "transparent",dpi=1200)
